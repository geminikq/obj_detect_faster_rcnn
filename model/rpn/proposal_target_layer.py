...
def proposal_target_layer(rpn_rois, gt_boxes, _num_classes, cfg, verbose=0):
    """
    ...
    num_images = 1
    rois_per_image = cfg_fix.TRAIN_BATCH_SIZE / num_images
    fg_rois_per_image = np.round(cfg_fix.TRAIN_FG_FRACTION * rois_per_image)

    # Sample rois with classification labels and bounding box regression
    # targets
    labels, rois, bbox_targets, bbox_inside_weights = _sample_rois(
        all_rois, gt_boxes, fg_rois_per_image, rois_per_image, _num_classes, cfg_fix)
...
def _get_bbox_regression_labels(bbox_target_data, num_classes, weight):
    ...
    for ind in inds:
        ...
        bbox_inside_weights[ind, start:end] = weight
